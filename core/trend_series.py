        ...
        series.append(
            {
                ...
                "observability_readiness_score": semantic_metrics.get(
...
def build_project_change_summary(artifacts_dir, limit: int = 2) -> dict[str, Any]:
    ...
    latest = {
        item.get("project_name"): item for item in history[-1]["snapshot"].get("projects", [])
    }
    previous = {
        item.get("project_name"): item for item in history[-2]["snapshot"].get("projects", [])
    }
            ...
            changes.append(
                {
                    ...
                    "change_type": "added",
                    "detail": "Project appeared in latest snapshot.",
                }
            )
            continue
            ...
            changes.append(
                {
                    "project_name": project_name,
                    "change_type": "removed",
                    "detail": "Project missing from latest snapshot.",
                }
            )
            continue
            ...
            changes.append(
                {
                    "id": repo_id,
                    "change_type": "added",
                    "detail": "Repository appeared in latest snapshot.",
                }
            )
            continue
        if not latest_repo:
            changes.append(
                {
                    "id": repo_id,
                    "change_type": "removed",
                    "detail": "Repository missing from latest snapshot.",
                }
            )
            continue
