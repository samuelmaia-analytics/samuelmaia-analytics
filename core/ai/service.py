...
class AnalyticsGenAIService:
    ...
    def build_suite(
        ...
        outputs = {
            ...
            "business_glossary_explanation": self.business_glossary_explanation(
                context, glossary_term
            ),
            "pipeline_dataset_description": self.pipeline_dataset_description(
    ...
    def pipeline_dataset_description(
        self, context: GenAIContext, dataset_name: str
    ) -> GenAIArtifact:
        focus = (
    ...
    def _stale_assets(self, context: GenAIContext) -> list[str]:
        stale: list[str] = []
        for asset_name, payload in (
            context.operational_context.get("freshness", {}).get("artifacts", {}).items()
        ):
    ...
    def _anomaly_action_hint(self, context: GenAIContext, concern: str) -> str:
        ...
        structural_summary = context.operational_context.get("snapshot_history", {}).get(
            "comparison_summary", ""
        )
    ...
    def _dataset_input_bullet(self, context: GenAIContext, dataset_name: str) -> str:
        ...
        if lineage:
            return (
                f"Primary inputs for `{dataset_name}`: {', '.join(str(item) for item in lineage)}."
            )
    ...
    def _dataset_output_bullet(self, context: GenAIContext, dataset_name: str) -> str:
        freshness = (
            context.operational_context.get("freshness", {})
            .get("artifacts", {})
            .get(dataset_name, {})
        )
